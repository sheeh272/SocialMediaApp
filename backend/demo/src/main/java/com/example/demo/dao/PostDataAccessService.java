...
public class PostDataAccessService implements PostDao{
    ...
    public  List<Post> getAllPosts(){
        String sql = "SELECT users.id, name, birthday, posts.id AS postId, author_id, contents FROM posts LEFT OUTER JOIN users ON posts.author_id = users.id";
        return jdbcTemplate.query(sql,new PostRowMapper());
    }

    @Override
    public List<Post> selectUserPosts(UUID user_id){
        String sql = "SELECT users.id, name, birthday, posts.id AS postId, author_id, contents FROM posts LEFT OUTER JOIN users ON posts.author_id = users.id WHERE posts.author_id = ?";
    ...
    public Optional<Post> selectPost(UUID id){
        String sql = "SELECT users.id, name, birthday, posts.id AS postId, author_id, contents FROM posts LEFT OUTER JOIN users ON posts.author_id = users.id WHERE posts.id = ?";
